...
Time: ${timeStr}`, mainMenu);
        ...
        if (replyText.includes('Enter payment amount for')) {
          ...
          if (isNaN(amount) || amount <= 0) {
             await sendTelegramMessage(chatId, `❌ Invalid amount. Transaction cancelled.`, mainMenu);
             return NextResponse.json({ ok: true });
          }
          ...
          // Verify we don't accidentally overpay (or allow overpay?) 
          // Qcontrol allows overpay (negative outstanding), but let's just record it.
          
          try {
             const { createLedgerEntryAndPayment } = require('@/lib/services/paymentService');
             await createLedgerEntryAndPayment({
                 businessId: business.id,
                 customerName: customer.name, // will resolve by name matching or phone
                 totalBilled: 0,
                 amountPaid: amount,
                 paymentMethod: 'Cash',
                 paymentStatus: 'Paid',
             ...
             await sendTelegramMessage(chatId, `✅ <b>Payment Recorded</b>\n\nCollected: ₹${amount}\nFrom: ${customer.name}\nRemaining Balance: ₹${Math.round(customer.outstanding_balance - amount)}`, mainMenu);
          } catch(e: any) {
             await sendTelegramMessage(chatId, `❌ Failed to record payment: ${e.message}`, mainMenu);
          }
          return NextResponse.json({ ok: true });
        }
        
        if (replyText.includes('Enter charge amount for')) {
            ...
            const reason = parts.slice(1).join(' ').trim() || 'Manual Charge';
            ...
            const msg = `Please confirm:\n\nMember: <b>${customer.name}</b>\nAmount to Add: <b>₹${amount}</b>\nReason: <b>${reason}</b>\nCurrent Outstanding: <b>₹${Math.round(customer.outstanding_balance)}</b>\nNew Outstanding: <b>₹${Math.round(customer.outstanding_balance) + amount}</b>`;
            
            const buttons = [
                [{ text: `✅ Confirm Charge`, callback_data: `qkhata_addcharge_confirm_${customer.id}_${amount}_${reasonB64}` }],
      ...
      if (text === '▶️ Start Session') {
        ...
        await sendTelegramMessage(chatId, 'Who is playing?', { inline_keyboard: buttons });
      }
        ...
        msg += `Members Pending: <b>${customers.length}</b>\n\n`;
        
        const topCustomers = customers.slice(0, 15);
        const memberButtons = topCustomers.map(c => [{
            text: `👤 ${c.name} — ₹${Math.round(c.outstanding_balance)}`,
            callback_data: `qkhatamem_${c.id}`
        }]);
        
        await sendTelegramMessage(chatId, msg + `Select a member to view details or collect payment:`, { inline_keyboard: memberButtons });
        return NextResponse.json({ ok: true });
      }
      else {
        // Main Menu
...
You can still access other businesses associated with your Telegram account.`, { reply_markup: { keyboard: kb, resize_keyboard: true } });
      ...
      if (callbackData.startsWith('qkhatamem_')) {
          ...
          if (!customer) {
             ...
             else await sendTelegramMessage(chatId, `❌ Member not found.`);
             return NextResponse.json({ ok: true });
          }
          
          // Get recent transactions
          const { data: payments } = await supabase.from('payments').select('*').eq('customer_id', customerId).order('created_at', { ascending: false }).limit(3);
          
          let msg = `👤 <b>QKhata: ${customer.name}</b>\n\n`;
          msg += `Outstanding Balance: <b>₹${Math.round(customer.outstanding_balance)}</b>\n\n`;
          
          if (payments && payments.length > 0) {
             msg += `<b>Recent Transactions:</b>\n`;
             payments.forEach((p: any) => {
                 msg += `• ${formatTimeReadable(p.created_at)}: ₹${p.amount} (${p.payment_method || 'Cash'})\n`;
             });
             msg += `\n`;
          }
          ...
          const buttons = [
             [
                 { text: `💸 Settle Full (₹${Math.round(customer.outstanding_balance)})`, callback_data: `qkhatasettlefull_${customer.id}` }
             ],
             [
                 { text: `💰 Partial Payment`, callback_data: `qkhatacollect_${customer.id}` },
                 { text: `➕ Add Charge`, callback_data: `qkhata_addcharge_init_${customer.id}` }
             ],
             [{ text: `↩️ Back`, callback_data: `qkhata_back` }]
          ];
      ...
      if (callbackData === 'qkhata_back') {
        ...
        if (!customers || customers.length === 0) {
           ...
           const topCustomers = customers.slice(0, 15);
           memberButtons = topCustomers.map(c => [{
               text: `👤 ${c.name} — ₹${Math.round(c.outstanding_balance)}`,
               callback_data: `qkhatamem_${c.id}`
           }]);
           msg += `Select a member to view details or collect payment:\n`;
        }
        
        memberButtons.push([{ text: `➕ Add Manual Charge`, callback_data: `qkhata_addcharge_menu` }]);
        
        if (messageId) {
           await editTelegramMessageText(chatId, messageId, msg, { inline_keyboard: memberButtons });
        } else {
      ...
      if (callbackData.startsWith('qkhatasettlefull_')) {
          ...
          const amount = Math.round(customer.outstanding_balance);
          
          const msg = `⚠️ <b>Confirm Full Settlement</b>\n\nAre you sure you want to settle the full amount of <b>₹${amount}</b> for ${customer.name}?`;
          
          const buttons = [
              [{ text: `✅ Yes, Settle ₹${amount}`, callback_data: `qkhataconfirm_${customer.id}` }],
              [{ text: `❌ Cancel`, callback_data: `qkhatamem_${customer.id}` }]
          ];
      ...
      if (callbackData.startsWith('qkhataconfirm_')) {
          ...
          const amount = customer.outstanding_balance;
          ...
          try {
             ...
             if (messageId) {
                ...
                await sendTelegramMessage(chatId, successMsg, mainMenu);
             }
          } catch(e: any) {
             const errMsg = `❌ Failed to settle payment: ${e.message}`;
             if (messageId) await editTelegramMessageText(chatId, messageId, errMsg, mainMenu);
             else await sendTelegramMessage(chatId, errMsg, mainMenu);
          }
          return NextResponse.json({ ok: true });
      }
      ...
      if (callbackData === 'qkhata_addcharge_menu') {
          ...
          let msg = `➕ <b>Add Manual Charge</b>\n\nSelect a member to add a charge to:\n`;
          
          const memberButtons = customers.map(c => [{
              text: `👤 ${c.name} — Bal: ₹${Math.round(c.outstanding_balance)}`,
              callback_data: `qkhata_addcharge_init_${c.id}`
          }]);
          
          memberButtons.push([{ text: `↩️ Back`, callback_data: `qkhata_back` }]);
      ...
      if (callbackData.startsWith('qkhata_addcharge_init_')) {
          ...
          const msg = `💰 <b>Enter charge amount for ${customer.name}</b>\n\nID: addcharge_${customer.id}\n\n(Reply with Amount and Reason, e.g. "500 Food" or just "500")`;
          await sendTelegramMessage(chatId, msg, { force_reply: true });
          return NextResponse.json({ ok: true });
      }

      if (callbackData.startsWith('qkhata_addcharge_confirm_')) {
          const parts = callbackData.replace('qkhata_addcharge_confirm_', '').split('_');
          const customerId = parts[0];
          const amount = parseFloat(parts[1]);
          const reasonB64 = parts[2];
          ...
          try {
             // We update balance safely by fetching latest inside paymentService.
             const { createLedgerEntryAndPayment } = require('../../../lib/services/paymentService');
             ...
             if (messageId) await editTelegramMessageText(chatId, messageId, `❌ Failed to add charge: ${e.message}`);
             else await sendTelegramMessage(chatId, `❌ Failed to add charge: ${e.message}`, mainMenu);
          }
          return NextResponse.json({ ok: true });
      }
      ...
      if (callbackData === 'start_member_init' || callbackData.startsWith('start_mem_page_')) {
        ...
        // Fetch from memberships table to sync perfectly with Dashboard's Member profiles
        ...
        const memberButtons = displayMembers.map(c => [{
            text: `👤 ${c.name}`,
      ...
      if (callbackData.startsWith('start_mem_exec_')) {
        ...
        const { data: member } = await supabase.from('memberships').select('name').eq('id', memberId).single();
        if (!member) {
            if (messageId) await editTelegramMessageText(chatId, messageId, `❌ Member not found.`);
            return NextResponse.json({ ok: true });
        }
      ...
      if (callbackData.startsWith('start_table_')) {
        ...
        if (action) {
          ...
          try {
            ...
            if (action === 'confirm_playing') {
              ...
              if (updatedSession) {
                ...
                const preferences = business.pricing_rules?.globalSettings?.preferences || {};
                if (updatedSession.member_id && preferences.enable_qkhata !== false) {
                    buttons[0].splice(1, 0, { text: `📒 QKhata`, callback_data: `qkhatainit_${updatedSession.id}` });
                }
          ...
          const { data: membership } = await supabase.from('memberships').select('name, mobile').eq('id', session.member_id).single();
          if (!membership) {
              if (messageId) await editTelegramMessageText(chatId, messageId, `❌ Member not found.`);
              else await sendTelegramMessage(chatId, `❌ Member not found.`);
              return NextResponse.json({ ok: true });
          ...
          // Also fetch their current outstanding balance if they have a ledger entry
          const { data: customerData } = await supabase.from('customers')
              .select('*')
              .eq('business_id', business.id)
              .or(`name.ilike.${membership.name},phone.eq.${membership.mobile}`)
              .limit(1)
              .single();
              
          const member = customerData || { name: membership.name, outstanding_balance: 0 };
          ...
          if (!session.member_id) return NextResponse.json({ ok: true });
          ...
          try {
             ...
             // Fetch the freshly updated customer balance (auto-created if it didn't exist)
             const { data: updatedMember } = await supabase.from('customers')
                .select('*')
                .eq('business_id', business.id)
                .or(`name.ilike.${membership.name},phone.eq.${membership.mobile}`)
                .limit(1)
             ...
             const member = { name: membership.name };

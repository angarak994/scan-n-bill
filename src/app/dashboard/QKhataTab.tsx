...
export default function QKhataTab({ businessId }: { businessId: string }) {
    ...
    const handleSendSmsReminder = async (e: React.MouseEvent, customer: any) => {
        e.stopPropagation();
        const toastId = toast.loading('Sending SMS reminder...');
        try {
            const messageTemplate = "Hi {{name}}, Your Qcontrol balance of Rs.{{outstanding}} is due today. Please clear it.";
            ...
            if (data.failureCount > 0) throw new Error(data.errors?.[0]?.error || 'Failed to send');

            toast.success('SMS reminder sent!', { id: toastId });
        } catch (error: any) {
            console.error('SMS Reminder error:', error);
            toast.error(error.message || 'Failed to send SMS reminder', { id: toastId });
        }
    };
                                                ...
                                                <div className="flex items-center justify-end gap-3">
                                                    ...
                                                    {Number(c.outstanding_balance) > 0 && c.phone && (
                                                        <div className="flex items-center gap-1">
                                                            <button 
                                                                onClick={(e) => handleSendSmsReminder(e, c)}
                                                                className="px-2 py-1 bg-accent/10 text-accent rounded hover:bg-accent/20 transition-colors shadow-sm flex items-center gap-1 border border-accent/20"

      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto mb-20">
        {pricingTiers.map((tier, idx) => (
          <ScrollReveal key={idx} animation={tier.isRecommended ? 'scale' : 'fade-up'} delay={tier.isRecommended ? 300 : idx * 150} className={tier.isRecommended ? 'z-10' : ''}>
            <div 
              className={`relative bg-bg-surface rounded-2xl border ${tier.isRecommended ? 'border-accent shadow-[0_0_30px_rgba(16,185,129,0.15)] scale-100 lg:scale-105 h-full' : 'border-border-light shadow-lg h-full'} p-6 sm:p-8 flex flex-col hover-lift`}
            >
            {tier.isRecommended && (
            ...
            <Link 
              href="/register" 
              className={`w-full py-3.5 rounded-xl font-bold text-center transition-all mt-auto ${tier.isRecommended ? 'bg-accent text-white hover:bg-accent/90 shadow-lg shadow-accent/20' : 'bg-bg-base text-text-primary border border-border-theme hover:bg-border-light hover:border-accent/50'}`}
            >
              {tier.ctaText}
            </Link>
          </div>

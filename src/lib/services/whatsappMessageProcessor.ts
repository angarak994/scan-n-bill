    ...
    if (current_step === 'AWAITING_MODIFY') {
      if (incomingText.startsWith('cancel_')) {
       ...
       if (incomingText.startsWith('date_')) {
          ...
          const availableTables = (business.tables || []).filter((t: any) => ((t as any).game_type || 'pool').toLowerCase() === context.game_type);
       ...
       if (customerName) {
         ...
         // Idempotency: Double check availability one last time (RACE CONDITION PREVENTION)
         // In production, this should ideally be an atomic INSERT ... WHERE NOT EXISTS or a server-side lock.
         // Here we use the double check, but wrap the booking creation in a retry/lock logic or assume it is fast.
